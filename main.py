...
class ProductionAstroSim(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Group 1: Experiment Scenario
        sys_group = QGroupBox("1. Experiment Mode Selection")
        sys_layout = QVBoxLayout()
        self.combo_system = QComboBox()
        self.combo_system.addItems([
            "Solar System & Comets (JPL Mode)",
            "Kepler-16 Binary Star System",
            "TRAPPIST-1 Habitable Zone Comparison"
        ])
        self.combo_system.currentIndexChanged.connect(self.on_system_change)
        sys_layout.addWidget(self.combo_system)
        sys_group.setLayout(sys_layout)

        # Group 2: Camera & Tracking Controls
        cam_group = QGroupBox("2. Camera & Target Tracking")
        cam_layout = QVBoxLayout()

        btn_layout = QHBoxLayout()
        btn_cam_3d = QPushButton("3D Perspective")
        btn_cam_3d.clicked.connect(lambda: self.reset_camera_target(distance=15, elev=30, azim=45))
        btn_cam_top = QPushButton("Top View")
        ...
        track_label = QLabel("Focus Target:")
        self.combo_target = QComboBox()
        self.combo_target.addItem("🌐 System Center", userData=None)
        ...
        # Group 3: Observation Toggles
        opt_group = QGroupBox("3. Observation Toggles")
        opt_layout = QVBoxLayout()
        
        self.chk_grid = QCheckBox("Show Reference Grid")
        self.chk_grid.setChecked(True)
        self.chk_grid.toggled.connect(self.on_grid_toggle)

        self.chk_orbits = QCheckBox("Show Orbit Lines")
        self.chk_orbits.setChecked(True)
        self.chk_orbits.toggled.connect(self.on_orbits_toggle)

        self.chk_hz = QCheckBox("Show Habitable Zone")
        self.chk_hz.toggled.connect(self.on_hz_toggle)

        self.chk_cb = QCheckBox("Colorblind Friendly Mode")
        ...
        play_group = QGroupBox("4. Simulation & Time Control")
        ...
        # Group 5: Telemetry Panel
        info_group = QGroupBox("5. Live Telemetry & System Info")
    ...
    def update_telemetry_panel(self):
        ...
        if not target_name:
            if self.current_system == "solar_system":
                self.info_box.setText("[Solar System Mode]\nIncludes eight major planets, Pluto with high inclination, periodic comet 1P/Halley, and hyperbolic comet 31/ATLAS.\n\n💡 Tip: Select a celestial body in the dropdown above to track it and view live telemetry.")
            elif self.current_system == "kepler_16":
                self.info_box.setText("[Kepler-16 System]\nA circumbinary planet system where exoplanet Kepler-16 b orbits two central stars.")
            elif self.current_system == "trappist_1":
                self.info_box.setText("[TRAPPIST-1 System]\nContains 7 terrestrial planets. Check 'Show Habitable Zone' to highlight planets e, f, and g.")
            return
        ...
        if data:
            ...
            <div style='font-family: sans-serif; font-size: 12px;'>
                ...
                <p style='margin: 3px 0;'><b>📍 Position (3D):</b><br>
                <code style='color: #FACC15;'>X:{data['pos'][0]:.2f} | Y:{data['pos'][1]:.2f} | Z:{data['pos'][2]:.2f}</code></p>
                
                <p style='margin: 5px 0;'><b>📏 Distance (r):</b> <span style='color: #4ADE80; font-weight: bold;'>{data['r']:.3f} AU</span></p>
                <p style='margin: 5px 0;'><b>⚡ Velocity (v):</b> <span style='color: #F87171; font-weight: bold;'>{data['v']:.2f} km/s</span></p>
                <p style='margin: 5px 0;'><b>⏱ Orbital Period (T):</b> <span style='color: #E0E7FF;'>{data['period']}</span></p>
                
                <hr style='border: 1px solid #334155; margin: 8px 0;'>
                <p style='margin: 3px 0;'><b>📐 Semi-major Axis (a):</b> {data['a']:.2f} AU</p>
                <p style='margin: 3px 0;'><b>🌀 Eccentricity (e):</b> {data['e']:.3f} ({'Elliptical' if data['e'] < 1 else 'Hyperbolic Escape'})</p>
            </div>
            """
    ...
    def build_solar_system(self):
        star_cfg = self.config["solar_system"].get("star", {"name": "Sun", "radius": 0.22, "color": [1, 0.9, 0, 1]})
        self.renderer.render_star(
            radius=star_cfg.get("radius", 0.22), 
            color=star_cfg.get("color", [1, 0.9, 0, 1]), 
            name=star_cfg.get("name", "Sun")
        )
    ...
    def build_trappist1_system(self):
        ...
        star_cfg = data.get("star", {"name": "TRAPPIST-1", "radius": 0.3, "color": [1, 0.3, 0.1, 1]})
        self.renderer.render_star(
            radius=star_cfg.get("radius", 0.3), 
            color=star_cfg.get("color", [1, 0.3, 0.1, 1]), 
            name=star_cfg.get("name", "TRAPPIST-1")
        )
    ...
    def update_time_display(self):
        ...
        self.lbl_clock.setText(f"🗓 {date_str}\n⏱ Elapsed: {int(self.sim_days)} days ({years:.2f} yrs)")
